...
get_popfit <- function() {
  ...
  y_predict <- predict(popfit)
  
  
  ##	For continuous regression, plot observed vs. predicted:
  plot(x=y_data, y=y_predict, 
       ylim=c(min(y_data),max(y_data)), xlim=c(min(y_data),max(y_data)),
       main= paste0("SMAE=", mean(abs(y_data- y_predict))/mean(abs(y_data))))
